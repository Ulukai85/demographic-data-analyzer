...
def calculate_demographic_data(print_data=True):
    ...
    num_min_workers = df[(df['hours-per-week'] == 1) & (df['salary'] == '>50K')].size

    rich_percentage = num_min_workers / df[df['hours-per-week'] == 1].size *100

    # What country has the highest percentage of people that earn >50K?
    high_earners = df[df.salary == '>50K']
    high_earners_per_country = high_earners.groupby(['native-country']).size()
   
    country_size = df.groupby(['native-country']).size()
    country_percentage = country_size.combine(high_earners_per_country, lambda a, b: b/a * 100)
